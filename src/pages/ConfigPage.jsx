...
export default function ConfigPage() {
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        if (data) setCfg((prev) => deepMerge(prev, data));
      } catch (e) {
        console.error(e);
      } finally {
        setLoading(false);
  ...
  const onCorsAdd = () => setCfg((prev) => ({ ...prev, system: { ...prev.system, cors: [...(prev.system.cors||[]), ""] } }));
  const onCorsEdit = (i, val) => setCfg((prev) => {
    const cors = [...(prev.system.cors || [])]; cors[i] = val; 
    return { ...prev, system: { ...prev.system, cors } };
  });
  const onCorsDel = (i) => setCfg((prev) => {
    const cors = [...(prev.system.cors || [])]; cors.splice(i, 1); 
    return { ...prev, system: { ...prev.system, cors } };
  });
  ...
  if (loading) return <div className="main"><div className="card">Cargando configuración…</div></div>;

  return (
    <div className="main" style={{ display: "grid", gap: 16 }}>
      <h2>Configuraciones del sistema y robot</h2>
              ...
              <div style={{ display: "grid", gap: 8 }}>
                {(cfg.system.cors || []).map((url, i) => (
                  <div key={i} style={{ display: "flex", gap: 8 }}>
                    ...
                    />
                    <button type="button" className="btn" onClick={() => onCorsDel(i)}>Eliminar</button>
                  </div>
                ))}
                <button type="button" className="btn" onClick={onCorsAdd}>Agregar origen</button>
              </div>
            </div>
          </div>
        </section>
...
function setAt(obj, pathArr, value) {
  ...
  cur[pathArr[pathArr.length - 1]] = typeof value === "string" && !isNaN(value) && value.trim() !== ""
    ? Number(value)
    : value;
  return copy;
}
